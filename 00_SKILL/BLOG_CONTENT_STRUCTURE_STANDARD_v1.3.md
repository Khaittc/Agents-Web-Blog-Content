# BLOG CONTENT STRUCTURE STANDARD v1.3
...
IMAGE_SPECIFICATION_AND_PROMPT_SKILL_v1.1.md
```

Blog Standard kiểm soát **cấu trúc, vai trò và cách trình bày nội dung**. IEEE Master Suite kiểm soát citation/reference. LaTeX Formula Skill kiểm soát cú pháp, biến, đơn vị và kiểm tra công thức. Image Specification Skill kiểm soát đặc tả ảnh, prompt AI và khung placeholder CKEditor.
...
- Hỗ trợ công thức, bảng kỹ thuật và tối thiểu 1 hình, tối đa 3 hình ảnh trong phần nội dung.
...
```text
Tối thiểu 1 hình đến tối đa 3 hình (1 ≤ n ≤ 3)
```

Mỗi bài viết kỹ thuật bắt buộc phải có **ít nhất 1 hình ảnh nội dung (Content Image)** để trực quan hóa nguyên lý kỹ thuật, sơ đồ hệ thống, đồ thị phân tích hoặc quy trình vận hành. Tuyệt đối không xuất bản bài viết kỹ thuật mà không có hình ảnh nội dung nào.

Mỗi hình phải có mục đích kỹ thuật rõ ràng, vị trí đặt hợp lý trong luồng nội dung và chú thích (caption) phân tích chi tiết.

### IMAGE_1 (Bắt buộc tối thiểu)

Ưu tiên:

- Giải thích nguyên lý kỹ thuật.
- Vị trí đo lường / Điểm đấu nối.
- Sơ đồ khái niệm / Mô hình hình học.
- Quy trình vận hành cốt lõi.

### IMAGE_2 (Tùy chọn khuyến nghị nếu cần so sánh)

Ưu tiên:

- So sánh kỹ thuật (trước vs sau, tuyến tính vs phi tuyến).
- Phân tích dữ liệu đo lường thực tế.
- Biểu đồ sóng / Đồ thị phổ tần số.
- Sơ đồ nguyên nhân - kết quả.

### IMAGE_3 (Tùy chọn khuyến nghị nếu có workflow phức tạp)

Ưu tiên:

- Bảng checklist kiểm tra hiện trường.
- Lưu đồ ra quyết định xử lý sự cố (Troubleshooting Workflow).
- Sơ đồ phối hợp giải pháp kỹ thuật tổng thể.
- Tổng hợp ma trận lựa chọn phương án.

Mỗi hình ảnh phải phục vụ việc giải thích chuyên môn, không tạo hình ảnh chỉ để trang trí hoặc gây xao nhãng.
