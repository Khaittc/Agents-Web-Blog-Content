...
**Version**: 1.0  
**Mã tài liệu**: `IEEE_04_CITATION_AUDIT_PROTOCOL_v1.0`  
...
Tech Review Agent đóng vai trò là "người gác cổng" (Quality Gatekeeper) cuối cùng trước khi một bài viết được chuyển sang xuất bản. Mọi sai sót về link ảo, tác giả giả mạo, trích dẫn lệch sự thật hoặc sai cấu trúc IEEE đều phải bị chặn lại ở bước này.
...
│ (Trang, Bảng, Eq.)   │      │ ([1], [2], cấm [1-3])│      │ Đặt tên chuẩn IEEE   │
└──────────────────────┘      └──────────────────────┘      └──────────────────────┘
```
...
| **Gate 4: Cấu trúc Đặt tên IEEE** | Kiểm tra thứ tự và kiểu chữ (in nghiêng vs ngoặc kép) theo chuẩn IEEE | So khớp với mẫu chuẩn tại `IEEE_03_REFERENCE_NAMING_AND_CKEDITOR_STYLE_SKILL`. | Standards/Manuals in nghiêng đầu; Reports/Blogs/Papers trong ngoặc kép; đầy đủ ngày truy cập và URL. | Sửa lại đúng cấu trúc IEEE tương ứng của loại hình đó. |
| **Gate 5: Cú pháp Ngoặc vuông** | Kiểm tra cách viết trích dẫn trong văn bản | Quét toàn bộ bài viết tìm các ký tự `[` và `]`. | Số thứ tự tăng dần liên tục từ `[1]` đến `[n]`; trích dẫn nhiều nguồn dùng `[1], [2]`; tuyệt đối không có `[1]–[3]`. | Yêu cầu Drafting Agent tách riêng từng ngoặc vuông. |
...
- [x] **Gate 5 - Cú pháp Ngoặc vuông**: 100% trích dẫn dùng cặp ngoặc riêng biệt `[1], [2], [3]`, không có gạch nối `[1]–[3]`. Đánh số tuyến tính từ [1] đến [7].
- [x] **Gate 6 - Clickable Hyperlinks**: Mọi tài liệu trực tuyến trong file HTML đều đã bọc thẻ `<a>` có link bấm trực tiếp `target="_blank"`.
